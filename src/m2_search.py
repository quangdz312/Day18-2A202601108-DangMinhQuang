...
def segment_vietnamese(text: str) -> str:
    ...
    try:
        ...
        return text  # fallback khi underthesea không cài được
...
class BM25Search:
    ...
    def search(self, query: str, top_k: int = BM25_TOP_K) -> list[SearchResult]:
        ...
        # Lọc docs không liên quan bằng token overlap, KHÔNG bằng `score > 0`.
        # Lý do: BM25Okapi cho IDF = log((n-df+0.5)/(df+0.5)); với corpus rất nhỏ
        # (n=2, df=1) → log(1.0) = 0 → mọi score = 0 dù query khớp hoàn toàn.
        # Dùng `score > 0` sẽ trả về rỗng trong trường hợp đó (false negative).
        query_terms = set(tokenized_query)

        results = []
        for i in top_indices:
            if not query_terms & set(self.corpus_tokens[i]):
                continue  # không chia sẻ token nào → thực sự không liên quan
...
def reciprocal_rank_fusion(results_list: list[list[SearchResult]], k: int = 60,
    ...
    for result_list in results_list:
        for rank, result in enumerate(result_list):
            # Khoá dedup gồm cả source: hai chunk khác file có thể trùng text
            # (VD cùng một câu boilerplate) — gộp chúng sẽ mất evidence và
            # gán nhầm metadata của file xuất hiện trước.
            key = (result.metadata.get("source"), result.text)
            if key not in rrf_scores:
                rrf_scores[key] = {"score": 0.0, "result": result}
            # rank là 0-based → +1 để rank đầu tiên đóng góp 1/(k+1).
