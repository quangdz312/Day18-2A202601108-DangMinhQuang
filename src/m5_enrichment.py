...
def generate_hypothesis_questions(text: str, n_questions: int = 3) -> list[str]:
    ...
    # Extractive fallback: biến câu trong chunk thành dạng câu hỏi.
                    ...
                    {"role": "system", "content": """Phân tích đoạn văn và trả về JSON:
{
  ...
  "metadata": {"topic": "...", "entities": ["..."], "category": "policy|hr|it|finance", "language": "vi|en"}
    ...
    # Fallback KHÔNG gọi API — dùng extractive thuần.
    # ⚠️ Không gọi summarize_chunk()/generate_hypothesis_questions() ở đây: khi có
    # API key mà combined call lỗi, hai hàm đó sẽ tự thử gọi OpenAI lần nữa
    # → 3 calls/chunk, phá vỡ cam kết "1 API call/chunk" của combined mode.
    sentences = [s.strip() for s in text.replace("\n", " ").split(". ") if s.strip()]
